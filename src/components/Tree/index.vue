 ...
 * @LastEditors: liuYang
 * @LastEditTime: 2021-04-19 16:23:11
 * @MustParam: 必传参数
    treeData 要渲染的树形数据
    labelKey 要渲染的文字的key
...
<template lang="pug">
  el-tree(
    ...
    span( class="custom-tree-node" slot-scope="{ node, data }" @click.stop="return false")
      el-checkbox.checkbox( :disabled="!data.checkable || disabled"  :indeterminate="data.isIndeterminate" :value="data.checked" @change="onClick(data)" )
    ...
    methods: {
      ...
      onClickLeaf(data) {
        if (data.checked) {
          ...
          if (data.from && data.from.size === 1) {
            console.log(`%c console.log  --------`, 'color:blue', 11111)
            console.log(`%c console.log  --------`, 'color:blue', Array.from(data.from.keys())[0])
          }
